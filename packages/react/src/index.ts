...
// The runtime ships as source files (under `src/runtime/`) and is compiled
// by the consumer's Vite + React plugin (e.g. @vitejs/plugin-react-oxc).
// This lets CSS modules and JSX work natively — no CSS-in-JS workaround,
// no Shadow DOM.
//
// Virtual modules drive the entry point so the consumer doesn't need to
// know any path. `virtual:stickynote-path:<rel>` resolves to the absolute
// source file; `load` reads it from disk directly so Vite's `fs.allow`
// restrictions don't block files outside the consumer's project root.
...
function overlayPlugin(
  ...
  return {
    name: "@vite-plugin-stickynote/react",
    ...
    async load(id) {
      if (id === VIRTUAL_MOUNT) {
        ...
        return [
          `import { mount } from "virtual:stickynote-path:runtime/overlay.tsx";`,
      ...
      // (e.g. `./components/App.tsx`), which Vite resolves to absolute
...
function readGitMeta(): { commit: string; dirty: boolean; root: string } {
  try {
    ...
    // `src/Foo.tsx` while GitHub expects `examples/react/src/Foo.tsx`.
